...
echo "Applico sudo hostnamectl set-hostname pegaso-$NAME_SUFFIX"
qm guest exec $VMID -- bash -c "sudo hostnamectl set-hostname pegaso-$NAME_SUFFIX"
...
qm guest exec $VMID -- bash -c "
    if [ -f '$FILE' ]; then
        sudo sed -i 's/evernode-template/pegaso-$NAME_SUFFIX\n127.0.0.1 pegaso-$NAME_SUFFIX.genesis-1-7.xyz/g' '$FILE' && \
